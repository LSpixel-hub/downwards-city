...
const isTerrainTile = (tile) => TERRAIN_TILES.has(tile);

const getTerrainDamage = (tile) => {
...
const getRandomTerrainTile = (level) => {
...
const isWalkable = (tile) => WALKABLE_TILES.has(tile);
...
const findUnexploredPassages = (revealedZones, map) => {
  const passages = [];
...
const renderTileData = (tile, biome, gemColor) => {
  switch (tile) {
    case TILE.VOID:
...
// ============================================
const PlasmaBackground = React.memo(({ biome }) => {
...
function DownwardsNeon() {
  ...
  const corridorDash = useCallback(
    (dx, dy) => {
      ...
      // Tiles that auto-pickup silently during dash
      const AUTO_PICKUP = new Set([TILE.GOLD, TILE.KEY]);
      // Tiles that stop BEFORE stepping on them (need confirm dialog)
      ...
      // Tiles that stop AFTER stepping on them (one-shot pickups)
